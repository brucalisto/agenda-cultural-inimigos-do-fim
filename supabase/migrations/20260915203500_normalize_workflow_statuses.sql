    ...
    case
      ...
      when count(*) filter (
        where review_status = 'necessita_revisao'
           or lower(coalesce(review_status, '')) in ('revisao', 'revisão', 'em_revisao', 'reprocessar')
           or lower(coalesce(review_status, '')) not in ('pendente', 'necessita_revisao', 'publicado', 'aprovado', 'ignorado', 'desativado')
      ...
      when count(*) filter (where review_status in ('publicado', 'aprovado')) > 0
